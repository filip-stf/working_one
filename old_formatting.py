...
try:
    ...
    st.markdown(
        ...
        /* The text field */
        #custom-input-container .input-box input {{
          flex: 1;
          margin-right: 8px;
          height: 40px;
        ...
        /* The send button */
        #custom-input-container .input-box button {{
          width: 40px;
          height: 40px;
          border: none;
          border-radius: 50%;
        ...
        /* Focus styles */
        #custom-input-container .input-box input:focus {{
          outline: none;
          border-color: #4A90E2;
    ...
    # Inject the custom input box HTML after the style section
    st.markdown("""
...
# Chat interaction
# --- Custom JS input handler for the HTML input box ---
from streamlit.components.v1 import html

def get_custom_input():
    # This will return the value entered in the custom input box when the send button is clicked
    return html('''
    <script>
    ...
    function sendInput() {
      ...
      if(val) {
        streamlitInput.postMessage({isStreamlitMessage: true, type: 'streamlit:setComponentValue', value: val}, '*');
        document.getElementById('chat-input').value = '';
      }
    }
    document.getElementById('chat-send').onclick = sendInput;
...
# --- Use the custom input handler in the app ---
if st.session_state.selected_characters:
    user_input = get_custom_input()
    if user_input:
        # Ensure user_input is a plain string
        if isinstance(user_input, dict) and 'value' in user_input:
            user_input = user_input['value']
        user_input = str(user_input)
        # Display the user's message before getting replies (as in rag_memory_old)
        st.session_state.messages.append(("You", user_input))
        try:
            loop = asyncio.new_event_loop()
            replies = loop.run_until_complete(
                run_chat(user_input, st.session_state.selected_characters)
            )
            loop.close()
